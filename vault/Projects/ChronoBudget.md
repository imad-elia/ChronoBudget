...
- Automated tests: `jest-expo` + `@testing-library/react-native`, run via `npm test` (83 tests across 10 suites). Covers `lib/detectCategory.ts`, `lib/recurrence.ts`, `lib/format.ts`, `store/useBudgetStore.ts`, `components/BentoCard.tsx`/`ExpenseInput.tsx`/`EditTransactionModal.tsx`/`RecurringModal.tsx`/`KeywordsModal.tsx`, and `db/database.ts` (schema migrations, CRUD, `processRecurring`) against a `sql.js`-backed mock of `expo-sqlite`. CI (`.github/workflows/ci.yml`) runs the suite + `tsc --noEmit` on every push/PR to `main`. See [[testing-strategy]].
...
- Testing: no E2E coverage (Detox/Maestro) of the full native app yet. See [[testing-strategy]] Known gaps.
